...
class Enemy extends PositionComponent with HasGameRef<TowerDefenseGame> {
  ...
  Color _getEnemyColor() {
    ...
    if (isFrozen) {
      return Color.alphaBlend(Colors.blue.withOpacity(0.5), baseColor);
    }
    if (isPoisoned) {
      return Color.alphaBlend(Colors.green.withOpacity(0.3), baseColor);
    }
    return baseColor;
  }
  ...
  void _healNearbyEnemies() {
    ...
    for (final enemy in gameRef.enemies) {
      if (enemy != this && (enemy.position - position).length <= config.healRadius) {
        enemy.health = (enemy.health + config.healAmount).clamp(0, enemy.maxHealth);
      }
    }
  }
